...
--PROCEDMIENTO PARA REPORTE TIENDA PRODUCTO
 alter procedure usp_rptProductoTienda(
 @IdTienda int = 0,
 @Codigo varchar(50)
 )
 as
...
begin
	...
	select top 5 year(FechaRegistro)[anio],MONTH(FechaRegistro)[dia],DATENAME(MONTH,FechaRegistro)[mes],COUNT(*)[TotalCosto] from [VENTA]
...
update VENTA set FechaRegistro = (dateadd(MONTH, 0, @fecha)) where IdVenta = 13
...
ALTER proc sp_GraficosDashBoard3
as
begin
	...
	Select TOP 12 p.Nombre[Descripcion],t.Stock from PRODUCTO p
	inner join PRODUCTO_TIENDA t on p.IdProducto = t.IdProducto
	where t.FechaRegistro between @fecha_minima and @fecha_maxima and Stock > 10
